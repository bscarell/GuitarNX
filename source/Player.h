...
/**
 * classe repr�sentant la potition des �toiles a l'�cran
 */
class Etoile {
public:
	int x, y;
...
public:
	...
	int showedPoints;// le score de joueur affich� a l'�cran
	int expectedPoints;// les prochains points qui vont etre gagn�
	int suite;//nombre de notes jou�e de suite actuellement
	int accuracy;//utilis� pour le score final, pr�cision en pourcent
	int longestStreak;//utilis� pour le score final, la plus longe suite

	//utilis� pour le menu de pause
	int motion; //utilis� pour l'equation diff�rentielle responsable du mouvement de texte
	int pauseSelection; //entr�e de munu s�l�ctionn� en pause
	...
	//float velocity;//TODO supprimer si non utilis�
	...
	bool touches[5];//pour savoir si les touches sont press�e a la d�rni�re boucle
	bool firetouches[5]; //pour savoir si les touches sont press�e dans la boucle boucle et pas avant
	...
	void update(glm::mat4 PV)
	{
		...
		if (state == play) {
			...
			touchesValSet();//on met a jour l'�tat des touches
			// gestions des notes
			{
				expectedPoints = 0;//on calcul dans cette variable les points qui vont etre gagn� mais qui ne sont pas comptabilis� dans la variable points
				for (int j = 0; j < 5; j++) {
					for (std::vector<Note*>::iterator it = notes[j].begin(); it != notes[j].end(); ++it) {
						Note* note = *it;
						...
						if (oggTime > (note->fin + TOLERENCE)) {// the note is completely passed
											//le +TOLERENCE sert a garder la note en mode press�e et non jou� assez longtemps pour que ce soit affich�
							if (!note->played && !note->missed && note->pressed) {
								note->played = true;
								note->released = true;
								note->releasetime = (int)oggTime;
				...
				for (int j = 0; j < 5; j++) {
					if (firetouches[j]) erreur();//une touche a �t� press� mais elle ne correspond a aucune note
					firetouches[j] = false;
				}
			}
			//gestion de la liste des �toiles
			{
				float delta = timeNow - timeLast;
