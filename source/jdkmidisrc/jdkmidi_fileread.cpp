...
namespace jdkmidi
{
  ...
  int MIDIFileRead::ReadMT( unsigned long type, int skip )	
  {
    ...
    if( type!=read )
    {
      ...
      mf_error((char*) "Error looking for chunk type" );
      return false;
    }
    else
    {
...
//
  ...
  void MIDIFileRead::ReadTrack()	
  {
    ...
    while( to_be_read > 0 && !abort_parse )
    {
      ...
      if( sysexcontinue && c!=0xf7 )
        mf_error((char*)"Error after expected continuation of SysEx" );
      
      if( (c&0x80)==0 )
      {
        if( status==0 )
          mf_error( (char*)"Unexpected Running Status" );
        running=1;
  ...
  int MIDIFileRead::EGetC()	
  {
    ...
    if( c<0 )
    {
      mf_error((char*)"Unexpected Stream Error" );
      abort_parse=true;
      return -1;
    }
