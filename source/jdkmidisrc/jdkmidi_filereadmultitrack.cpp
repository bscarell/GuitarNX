...
namespace jdkmidi
{
  ...
  void    MIDIFileReadMultiTrack::mf_tempo( MIDIClockTime time, unsigned long tempo )	
  {
    ...
    float beats_per_second = 1e6	 // 1 million microseconds per second
      / (double)tempo;
    
    float beats_per_minute = beats_per_second * 60;
    
    tempo_bpm_times_32 = (unsigned long) (beats_per_minute * 32.0);
    
    MIDITimedMessage msg;
    
    msg.SetTempo32( tempo_bpm_times_32 );
    msg.SetTime( time );
  ...
  void    MIDIFileReadMultiTrack::mf_keysig(MIDIClockTime time, int, int )	
  {
